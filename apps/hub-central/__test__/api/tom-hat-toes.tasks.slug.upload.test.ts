...
import { POST, DELETE } from '@/app/api/tom-hat-toes/tasks/[slug]/upload/route';
...
describe('API Task Artifacts - Greffe et Dissolution de Brindilles (Fichiers & Sceau SHA-256)', () => {
  ...
  describe('POST /api/tom-hat-toes/tasks/[slug]/upload', () => {
  ...
  describe('DELETE /api/tom-hat-toes/tasks/[slug]/upload', () => {
    it('doit rejeter (403) en cas de tentative IDOR sur une URL étrangère', async () => {
      ...
      const req = new NextRequest('http://localhost/api/tom-hat-toes/tasks/ma-tache/upload', {
        method: 'DELETE',
