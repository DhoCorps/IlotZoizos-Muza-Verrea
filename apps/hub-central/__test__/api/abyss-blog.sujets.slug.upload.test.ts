...
import { POST, DELETE } from '@/app/api/abyss-blog/sujets/[slug]/upload/route';
...
describe('Route API : Abyss Upload & Delete Sujet Media & Sceau SHA-256 (POST / DELETE)', () => {
  ...
  it('DELETE - doit rejeter (403) en cas de tentative IDOR sur une URL étrangère normalisée', async () => {
    ...
    const req = new NextRequest('http://localhost/api/abyss-blog/sujets/mon-sujet/upload?url=https://cdn.ilot/url-etrangere.jpg', {
      method: 'DELETE',
    });
  ...
  it('DELETE - doit purger le média, passer par l\'Orchestrator et invalider le cache', async () => {
    ...
    const req = new NextRequest('http://localhost/api/abyss-blog/sujets/mon-sujet/upload?url=https://cdn.ilot/media.jpg', {
      method: 'DELETE',
    });
