...
import { POST, DELETE } from '@/app/api/tom-hat-toes/projects/[slug]/upload/route';
...
// 🧪 SUITE DE TESTS
// -------------------------------------------------------------------------
describe('Route API : Project Attachments & Sceau SHA-256 (POST / DELETE /api/tom-hat-toes/projects/[slug]/upload)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    delete global.__mockUser;

    vi.spyOn(storageService, 'generateKey').mockReturnValue('ilot-zoizos/fr/tom-hat-toes/projects/proj-1/attachments/test.pdf');
  ...
  describe('DELETE - Purge d\'un artefact', () => {
    it('doit refuser (403) si l\'artefact n\'appartient pas au projet (Protection IDOR)', async () => {
      ...
      const req = new NextRequest('http://localhost/api/tom-hat-toes/projects/mon-chantier/upload', {
        method: 'DELETE',
