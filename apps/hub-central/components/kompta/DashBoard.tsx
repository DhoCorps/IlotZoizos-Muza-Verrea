...
export function KomptaDashboard({ userUid }: KomptaDashboardProps) {
  ...
  // 🚀 Harmonisation ERP : Les valeurs du summary et des entrées s'appuient sur amountCents / conversion en unités
  const { entries = [], summary = { totalCreditsCents: 0, totalDebitsCents: 0, netBalanceCents: 0, transactionCount: 0 } } = ledgerData || {};

  const totalCredits = (summary.totalCreditsCents || summary.totalCredits || 0) / 100;
  const totalDebits = (summary.totalDebitsCents || summary.totalDebits || 0) / 100;
  const netBalance = (summary.netBalanceCents || summary.netBalance || 0) / 100;
    ...
    <div className="max-w-6xl mx-auto space-y-8 p-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col justify-between space-y-4">
          ...
          <span className="text-2xl font-black font-mono text-emerald-400">+{totalCredits.toFixed(2)} €</span>
        </div>
        ...
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col justify-between space-y-4">
          ...
          <span className="text-2xl font-black font-mono text-rose-400">-{totalDebits.toFixed(2)} €</span>
        </div>
        ...
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col justify-between space-y-4">
          ...
          <span className={`text-2xl font-black font-mono ${netBalance >= 0 ? 'text-amber-300' : 'text-rose-400'}`}>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 backdrop-blur-md space-y-6">
        <h3 className="text-sm font-black uppercase tracking-wider text-slate-200 flex items-center gap-2">
          <Activity size={16} className="text-amber-400" /> Historique des Écritures ({summary.transactionCount || entries.length})
        </h3>

        <div className="space-y-3">
          {entries.map((entry: any, index: number) => {
            // S'assure de l'utilisation de amountCents si présent
            const cents = entry.amountCents ?? (typeof entry.amount === 'number' ? entry.amount * 100 : 0);
            return (
              <div key={entry.entryUid || entry._id || index} className="p-4 bg-slate-950/60 border border-slate-800/80 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all hover:border-amber-500/30">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-slate-500">{new Date(entry.createdAt || Date.now()).toLocaleString()}</span>
                  </div>
                  <p className="text-sm font-bold text-slate-200">{entry.description}</p>
                  <p className="text-[10px] font-mono text-slate-500 truncate max-w-md">Hash : {entry.entryHash || 'N/A'}</p>
                </div>

                <div className={`text-base font-black font-mono ${entry.type === 'CREDIT' ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {entry.type === 'CREDIT' ? '+' : '-'}{(cents / 100).toFixed(2)} {entry.currency || 'EUR'}
                </div>
              </div>
            );
          })}
