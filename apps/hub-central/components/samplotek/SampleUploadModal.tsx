...
export const SampleUploadModal: React.FC<SampleUploadModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Sceau SHA-256 forgé ! Sample gravé dans la Matrice. 🛡️💿');
      onSuccess();
      onClose();
    } catch (err: any) {
      ...
      <div className="w-full max-w-xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-black uppercase tracking-wider text-slate-100">Graver & Sceller un Sample</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[80vh] custom-scrollbar">
            ...
            <div className="border-2 border-dashed border-slate-700 hover:border-red-500/50 rounded-2xl p-6 text-center transition-colors cursor-pointer relative bg-slate-950/30">
              ...
              <div className="flex flex-col items-center space-y-2">
                ...
                <span className="text-[10px] font-mono text-slate-500 flex items-center justify-center gap-1">
                  <Shield size={10} /> Empreinte cryptographique générée à l'upload
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <input 
                type="text" required value={style} onChange={(e) => setStyle(e.target.value)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input 
                type="number" required min={40} max={300} value={tempoBpm} onChange={(e) => setTempoBpm(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs font-mono text-slate-200 focus:outline-none focus:border-red-500"
              />
            </div>
            <div className="space-y-2">
              ...
              <input 
                type="text" required value={musicalKey} onChange={(e) => setMusicalKey(e.target.value)}
          ...
          <div className="space-y-3 pt-2 border-t border-slate-800">
            ...
            <div className="space-y-2 bg-slate-950/40 p-4 rounded-2xl border border-slate-800/80">
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={allowRadio} onChange={(e) => setAllowRadio(e.target.checked)} className="accent-red-600 w-4 h-4 rounded" />
                <span className="text-xs font-mono text-slate-300">Diffuser sur la Radio de l'Îlot</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={allowBlindTest} onChange={(e) => setAllowBlindTest(e.target.checked)} className="accent-red-600 w-4 h-4 rounded" />
                <span className="text-xs font-mono text-slate-300">Intégrer aux Blind-Tests</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={allowShowcase} onChange={(e) => setAllowShowcase(e.target.checked)} className="accent-red-600 w-4 h-4 rounded" />
                <span className="text-xs font-mono text-slate-300">Autoriser le Diaporama Universel</span>
              </label>
            </div>
          </div>

          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl font-mono text-xs text-slate-400 hover:text-white transition-colors">
              Annuler
            </button>
            <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-black uppercase tracking-widest text-xs rounded-xl shadow-[0_0_20px_rgba(220,38,38,0.3)] transition-all flex items-center gap-2">
              {isSubmitting ? <Loader2 className="animate-spin" size={16} /> : <Upload size={16} />}
              {isSubmitting ? 'Scellement...' : 'Forger l\'Empreinte'}
            </button>
          </div>
