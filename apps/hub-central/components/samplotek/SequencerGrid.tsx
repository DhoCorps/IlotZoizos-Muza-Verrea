...
export const SequencerGrid: React.FC = () => {
  const { bpm, isPlaying, tracks, setBpm, setIsPlaying, toggleMute, setTrackVolume, unlockNextTrack } = useStudioStore();

  // Lancement du moteur audio Tone.js
  useOmniSamplerEngine(tracks, isPlaying, bpm);

  const handleUnlockAttempt = async () => {
    try {
      // Appel à l'API d'économie pour vérifier et dépenser les ressources de l'Alvéole
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-6 bg-slate-950 border border-slate-800 rounded-3xl shadow-2xl space-y-8 text-white">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 pb-6 border-b border-slate-800">
        <div className="space-y-1">
          ...
          <p className="text-xs font-mono text-slate-400">
            Moteur synchrone Tone.js • <span className="text-red-400 font-bold">{tracks.filter(t => !t.isLocked).length} Pistes Actives</span>
          </p>
        </div>
      ...
      <div className="p-4 bg-gradient-to-r from-red-950/30 via-slate-900 to-slate-900 border border-red-500/20 rounded-2xl flex items-center justify-between">
        ...
        <span className="text-xs font-mono font-bold text-red-400">{tracks.filter(t => !t.isLocked).length} / 8 Pistes</span>
      </div>
