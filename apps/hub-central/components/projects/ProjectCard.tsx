...
import { Paperclip, Zap, Layers, Plus, Trash2, Upload, Loader2, FileText, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react'; // 🪡 SUTURE : Icones pour la matrice d'upload
...
export function ProjectCard({ project, onEdit, onCreateTask, onDelete, onViewTasks }: ProjectCardProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 🌐 SUTURE VERROUILLÉE : Frapper la route d'API dynamique correspondante
      ...
      // 🪡 SUTURE DE REPLI SÉCURISÉ : Si l'API ne renvoie pas d'UID d'artefact, on le génère de façon bionique
      const newDoc = {
        uid: data.document?.uid || data.key || `doc_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
        name: file.name,
      ...
      // On enrichit dynamiquement la liste locale pour un rendu instantané
      ...
      <div>
        ...
        {isDrawerOpen && (
          ...
          >
            ...
            {/* Catalogue des documents scellés */}
            <div className="space-y-1.5 max-h-32 overflow-y-auto custom-scrollbar">
              {localDocuments.map((doc: any) => (
                ...
                >
                  ...
                  <ExternalLink size={10} className="opacity-0 group-hover/doc:opacity-100 transition-opacity text-slate-500 shrink-0" />
                </a>
              ))}
