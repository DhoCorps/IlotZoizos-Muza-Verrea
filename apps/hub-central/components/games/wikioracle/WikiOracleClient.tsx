...
export default function WikiOracleClient({ roomId, username }: WikiOracleClientProps) {
  ...
  useEffect(() => {
    ...
    const handleNewQuestion = (data: { round: number; questionTitle: string; options: string[]; initialHint: string; imageUrl?: string }) => {
      setCurrentQuestion({
    ...
    const handleNewHint = (data: { hintLevel: number; hintText: string }) => {
    ...
    socket.on('wikioracle:new-question', handleNewQuestion);
    socket.on('wikioracle:countdown', handleCountdown);
    socket.on('wikioracle:new-hint', handleNewHint);
    socket.on('wikioracle:feedback', handleFeedback);
    ...
    return () => {
      ...
      socket.off('wikioracle:new-question', handleNewQuestion);
      socket.off('wikioracle:countdown', handleCountdown);
      socket.off('wikioracle:new-hint', handleNewHint);
      socket.off('wikioracle:feedback', handleFeedback);
