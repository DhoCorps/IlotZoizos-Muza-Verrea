...
export function ProductComparator({ products, onClose, onRemove }: ProductComparatorProps) {
  ...
  return (
    <div 
      ...
      role="dialog" 
      aria-modal="true" 
      aria-label="Comparateur d'Artefacts"
    >
          ...
          <div className="flex gap-4 min-w-max">
            {products.map((product) => (
              <div key={product.uid} className="w-80 bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col relative group">
                ...
                <button 
                  aria-label={`Acquérir ${product.title}`}
                  className="mt-6 w-full py-3 bg-emerald-600/20 hover:bg-emerald-600 border border-emerald-500/50 hover:border-emerald-500 text-emerald-400 hover:text-white font-black uppercase text-[10px] tracking-widest rounded-xl transition-all flex items-center justify-center gap-2"
                >
