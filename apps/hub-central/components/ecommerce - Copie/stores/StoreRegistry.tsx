...
export const storeRegistry: BlockRegistry = {
  'product-hero': {
    ...
    defaultData: {
      ...
      priceCents: 4500, // 45.00 € stocké en centimes pour le backend
      priceShards: 120,
      category: 'CV_TEMPLATE'
    },
    renderView: ({ data }) => {
      // Conversion sécurisée des centimes en euros pour l'affichage visuel
      const displayEUR = ((data.priceCents || 0) / 100).toFixed(2);
      return (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-xs font-mono font-bold">
              <span className="text-white">{displayEUR} €</span>
    ...
    renderEditForm: ({ data, onChange }) => {
      // Valeur affichée en euros dans le champ de saisie
      const currentEUR = (data.priceCents || 0) / 100;
      return (
        <div className="space-y-3">
          <div>
            ...
            <div>
              ...
              <input 
                ...
                onChange={e => {
                  ...
                  // Conversion automatique et rigoureuse en centimes (priceCents) pour le backend
                  onChange({ ...data, priceCents: Math.round(eurValue * 100) });
                }}
                className="w-full bg-black/60 border border-white/10 p-2.5 rounded-xl text-xs text-white font-mono outline-none focus:border-[#E5484D]"
              />
            </div>
