    ...
    <div className="p-6 bg-black/40 border border-white/5 rounded-3xl backdrop-blur-xl flex flex-col justify-between space-y-4 hover:border-cyan-500/30 transition-all group">
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-1">
            {tags.map((t: string, idx: number) => (
              <span key={idx} className="text-[9px] font-mono px-2 py-0.5 rounded-md bg-white/5 text-slate-400 border border-white/5 flex items-center gap-1">
                <Tag size={10} /> {t}
              </span>
            ))}
          </div>
      ...
      <div className="space-y-4 pt-4 border-t border-white/5 flex items-center justify-between">
        <div>
          ...
          <span className="text-lg font-black text-white font-mono">{((product.priceCents || 0) / 100).toFixed(2)} €</span>
        </div>
