...
interface PapierAncreReaderProps {
  ...
  // Rétrocompatibilité props à plat pour les tests existants
  bookUid?: string;
  title?: string;
  author?: string;
  authorUid?: string;
...
export const PapierAncreReader: React.FC<PapierAncreReaderProps> = (props) => {
  // 🛡️ Double liaison : supporte à la fois l'objet `book` (SSR) et les props à plat (Tests)
  ...
  const handleSaveEmotionalHighlight = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedText) return;

    setIsSubmittingHighlight(true);
    try {
      const res = await fetch(`/api/bibliotek/${bookUid}/emotional-highlights`, {
        method: 'POST',
      ...
      toast.success("✨ Fulgurance et vibration transmises à l'auteur avec succès !");
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${themeStyles.bg} ${themeStyles.text} flex flex-col relative`}>
      ...
      {popupPosition && !isHighlightModalOpen && (
        <div className="fixed z-50 transform -translate-x-1/2 -translate-y-full mb-3" style={{ left: popupPosition.x, top: popupPosition.y }}>
          <button onClick={() => setIsHighlightModalOpen(true)} className="bg-[#E5484D] text-white px-4 py-2 rounded-xl text-xs font-sans font-bold shadow-2xl flex items-center gap-2 hover:bg-[#D43D42] transition-all scale-105">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="w-full max-w-lg bg-[#1A1D24] border border-[#30363D] rounded-3xl p-6 md:p-8 shadow-2xl relative space-y-6 font-sans">
            <div className="flex items-center justify-between pb-4 border-b border-[#30363D]">
              ...
              <button onClick={() => setIsHighlightModalOpen(false)} className="text-xs text-[#8B949E] hover:text-white"><X size={18} /></button>
            </div>
            ...
            <form onSubmit={handleSaveEmotionalHighlight} className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-[#8B949E] uppercase tracking-wider">Choisir une vibration (Émoji / Signature) :</label>
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={() => setIsHighlightModalOpen(false)} className="px-4 py-2 bg-transparent text-[#8B949E] hover:text-white text-xs font-medium">Annuler</button>
                <button type="submit" disabled={isSubmittingHighlight} className="px-6 py-2.5 bg-[#E5484D] hover:bg-[#D43D42] text-white font-black uppercase text-xs rounded-xl shadow-lg transition-all flex items-center gap-2 disabled:opacity-50">
                  {isSubmittingHighlight ? <Loader2 size={14} className="animate-spin" /> : 'Diffuser l\'écho'}
                </button>
              </div>
            </form>
          </div>
