// apps/hub-central/app/api/abyss/sujets/[slug]/upload/route.ts
...
export async function POST(
  ...
  try {
    ...
    // 2. 🛡️ Rate Limiting par IP contre le spam
    const clientIp = req.headers.get('x-forwarded-for') || '127.0.0.1';
    const { allowed } = await checkRateLimit(`upload-sujet-slug:${clientIp}`, 10, 60);
    if (!allowed) {
    ...
    // 3. Génération de la clé structurée unique pour le sujet/monologue
    const structuredKey = storageService.generateStructuredKey({
      inceptId: 'hub-central',
      locale: 'fr',
      entityType: 'projects', // ou section abysse
      entityId: slug,
    ...
    console.error('❌ [ABYSS SLUG UPLOAD ERROR] :', error);
...
export async function DELETE(
  ...
  try {
    ...
    console.log(`🗑️ [Abyss] Média purgé pour le sujet [slug: ${params.slug}]`);
