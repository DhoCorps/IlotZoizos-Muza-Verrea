...
function roomToRoomToSend(room, requestingPlayerId) {
...
function syncRoomFromManager(roomId, gameType) {
    let roomFromManager;
    switch (gameType) {
        case 'CrazyMorpion':
            roomFromManager = crazyMorpionManager.getRoom(roomId);
            break;
        case 'KoOonTreeZ':
            roomFromManager = koOonTreeZManager.getRoom(roomId);
            break;
        case 'AtomikKFardE':
            roomFromManager = atomikKFardEManager.getRoom(roomId);
            break;
        case 'CineMax':
            roomFromManager = cineMaxManager.rooms?.get(roomId);
            break;
        case 'SoonArt':
            roomFromManager = soonArtManager.getRoom(roomId);
            break;
        case 'GalakTK':
            roomFromManager = galakTKManager.getRoom(roomId);
            break;
        case 'PlumZee':
            roomFromManager = plumZeeManager.getRoom(roomId);
            break;
        case 'WikiOracle':
            roomFromManager = wikiOracleManager.getRoom(roomId);
            break;
    }
    if (roomFromManager) {
...
function notifyDisconnectToManager(gameType, socketId, roomId) {
    switch (gameType) {
        case 'CrazyMorpion':
            crazyMorpionManager.notifyPlayerDisconnect(socketId, roomId);
            break;
        case 'KoOonTreeZ':
            koOonTreeZManager.notifyPlayerDisconnect(socketId, roomId);
            break;
        case 'AtomikKFardE':
            atomikKFardEManager.notifyPlayerDisconnect(socketId, roomId);
            break;
        case 'PlumZee':
            plumZeeManager.notifyPlayerDisconnect(socketId, roomId);
            break;
        case 'WikiOracle':
            wikiOracleManager.notifyPlayerDisconnect(socketId, roomId);
            break;
    }
}
function deleteRoomFromManager(gameType, roomId) {
    switch (gameType) {
        case 'CrazyMorpion':
            crazyMorpionManager.deleteRoom(roomId);
            break;
        case 'KoOonTreeZ':
            koOonTreeZManager.deleteRoom(roomId);
            break;
        case 'AtomikKFardE':
            atomikKFardEManager.deleteRoom(roomId);
            break;
        case 'SoonArt':
            soonArtManager.deleteRoom(roomId);
            break;
        case 'GalakTK':
            galakTKManager.deleteRoom(roomId);
            break;
        case 'PlumZee':
            plumZeeManager.deleteRoom(roomId);
            break;
        case 'WikiOracle':
            wikiOracleManager.deleteRoom(roomId);
            break;
    }
}
...
async function bootstrapServer() {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('disconnect', () => {
            ...
            const allPlayersEffectivelyDisconnected = updatedRoomFromManager?.players.every((p) => {
                if (updatedRoomFromManager?.gameType === 'AtomikKFardE') {
                    const atomikPlayer = updatedRoomFromManager.players.find((ap) => ap.id === p.id);
                ...
                const timer = setTimeout(() => {
                    ...
                    const playersStillDisconnected = finalRoomState?.players.every((p) => {
                        if (finalRoomState.gameType === 'AtomikKFardE') {
                            const atomikPlayer = finalRoomState.players.find((ap) => ap.id === p.id);
                            return atomikPlayer && atomikPlayer.status === 'disconnected';
                        }
        ...
        socket.on('room:create', (payload) => {
            const { username, roomName, gameType, kooonTreezNbPlayer, kooonTreezMode, kooonTreezOption, kooonTreezLevel, kooonTreezSoloMode, atomikKFardENbPlayer, atomikKFardEMode, atomikKFardEOption, atomikKFardEGameStyle, cineMaxNbPlayer, cineMaxDifficultyRule, cineMaxTimePerRound, cineMaxScoreToWin, cineMaxMaxRounds, soonArtTotalTreasures, soonArtMaxCircles, galakTKGridWidth, galakTKGridHeight, galakTKTotalStars, galakTKMode, plumZeeMaxRounds, plumZeeTurnTimeLimit, choicesMode, theme } = payload;
            const roomId = Date.now().toString();
            let newRoomServer = null;
            let roomToSendToClient = null;
            const ownerPlayerId = socket.id;
            const atomikKFardEOptions = {
                ...
                switch (gameType) {
                    ...
                    case 'KoOonTreeZ': {
                        const options = {
                            ...
                            kooonTreezSoloMode: kooonTreezSoloMode || 'training'
                        };
                        const creator = {
                            id: ownerPlayerId,
                            username,
        ...
        socket.on('room:join', ({ roomId, username }) => {
            ...
            switch (room.gameType) {
                case 'CrazyMorpion':
                    crazyMorpionManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'KoOonTreeZ':
                    koOonTreeZManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'AtomikKFardE':
                    atomikKFardEManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'CineMax':
                    cineMaxManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'SoonArt':
                    soonArtManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'GalakTK':
                    galakTKManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'PlumZee':
                    plumZeeManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
                case 'WikiOracle':
                    wikiOracleManager.handlePlayerJoin(roomId, username, socket.id);
                    break;
            }
        ...
        socket.on('kooontreez:start-game', ({ roomId }) => {
            const room = rooms.get(roomId);
        ...
        socket.on('wikioracle:start-game', ({ roomId }) => {
            const room = rooms.get(roomId);
        ...
        socket.on('atomikkfarde:start-game', ({ roomId }) => {
            const room = rooms.get(roomId);
        ...
        socket.on('game:restart-request', ({ roomId }) => {
            const room = rooms.get(roomId);
            if (!room)
                return;
            try {
