...
import { CRAZYMORPION_SYMBOL_EMPTY, CRAZYMORPION_SYMBOL_PLUS, CRAZYMORPION_SYMBOL_MINUS, } from '@ilot/shared-core';
import { createEmptyCrazyMorpionGrid, checkCrazyMorpionWinner, checkCrazyMorpionDraw, makeCrazyMorpionMove, getRandomCrazyMorpionSymbol } from '@ilot/shared-core';
...
const crazyMorpionRooms = new Map();
...
export class CrazyMorpionManager {
    io; // Référence au serveur Socket.IO pour émettre des événements
     ...
     */
    createRoom(roomId, roomName, creatorPlayer) {
        const newRoom = {
            id: roomId,
            name: roomName,
            ...
            playerDisconnectTimers: new Map() // Initialiser la Map
        };
    ...
    handlePlayerJoin(roomId, username, newSocketId) {
        ...
        console.log(`[CrazyMorpionManager - DEBUG] Salon ${roomId}: Après consolidation. État des joueurs:`, room.players.map(p => ({ id: p.id, username: p.username, status: p.status, symbol: p.symbol })));
        ...
        if (connectedPlayersCount === room.maxPlayers) {
            ...
            if (activePlayers.length >= 2) { // S'assurer qu'il y a bien 2 joueurs actifs pour assigner les symboles
                if (Math.random() <= 0.5) {
                    activePlayers[0].symbol = CRAZYMORPION_SYMBOL_PLUS;
                    activePlayers[1].symbol = CRAZYMORPION_SYMBOL_MINUS;
                }
                else {
                    activePlayers[0].symbol = CRAZYMORPION_SYMBOL_MINUS;
                    activePlayers[1].symbol = CRAZYMORPION_SYMBOL_PLUS;
                }
            }
     ...
     */
    handleMakeMove(roomId, playerId, x, y, chosenSymbolFromClient) {
        const room = crazyMorpionRooms.get(roomId);
        ...
        if (roundWinnerId) {
            ...
            GameStatsService.recordMatch(matchData).then((success) => {
                if (success)
                    console.log(`[CrazyMorpionManager] Historique sauvegardé avec succès pour le salon ${roomId}`);
            });
            // =========================================================
        ...
        if (drawInfo && !winInfo.playerHasWon && !winInfo.ennemyHasWon) {
            ...
            const matchData = {
                ...
                players: room.players.map(p => ({
                    ...
                    specificStats: { symbolAssigned: p.symbol }
                })),
                matchMetadata: {
                    totalRounds: room.round,
                    draw: true
            ...
            GameStatsService.recordMatch(matchData).then((success) => {
                if (success)
                    console.log(`[CrazyMorpionManager] Historique (Match Nul) sauvegardé pour le salon ${roomId}`);
            });
            // =========================================================
            this.io.to(roomId).emit('game:over', this.roomToRoomToSend(room));
            this.io.emit('room:list', Array.from(crazyMorpionRooms.values()).map(this.roomToRoomToSend));
    ...
    handleRestartRequest(roomId, requestingPlayerId) {
        ...
        if (connectedPlayers.length === 2) {
            ...
            player1.symbol = CRAZYMORPION_SYMBOL_PLUS; // Assigner symboles pour restart
            player2.symbol = CRAZYMORPION_SYMBOL_MINUS; // Assigner symboles pour restart
                    ...
                    if (playerStillTempDisconnected) {
                        ...
                        if (currentRoom.currentTurnPlayerId === playerStillTempDisconnected.id && currentRoom.state === 'playing') {
                            ...
                            if (otherActivePlayer) {
                                ...
                                GameStatsService.recordMatch(matchData).then((success) => {
                                    if (success)
                                        console.log(`[CrazyMorpionManager] Historique d'interruption sauvegardé pour ${roomId}`);
                                });
                                // =========================================================
                            ...
                            if (winnerPlayer) {
                                ...
                                GameStatsService.recordMatch(matchData).then((success) => {
                                    if (success)
                                        console.log(`[CrazyMorpionManager] Historique forfaitaire sauvegardé pour ${roomId}`);
                                });
                            }
                            // =========================================================
