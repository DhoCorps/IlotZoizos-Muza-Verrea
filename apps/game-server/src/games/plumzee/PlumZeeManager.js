...
export class PlumZeeManager {
    ...
    handleMakeMove(roomId, playerId, move) {
        ...
        if (!room || room.state !== 'playing')
            throw new Error("Partie Plum'Zee inactive.");
        if (room.currentTurnPlayerId !== playerId) {
            throw new Error("Ce n'est pas votre tour de lancer les dés cosmiques !");
        }
        const player = room.players.find(p => p.id === playerId);
        if (!player)
            throw new Error("Joueur introuvable dans le salon.");
        ...
        if (move.action === 'SCORE_COMBINATION') {
            ...
            if (!combinationKey)
                throw new Error("Combinaison non spécifiée.");
            if (player.rollsLeft === 3) {
            ...
            player.totalScore = Object.values(player.scoreSheet).reduce((acc, val) => acc + (val || 0), 0);
    ...
    handleRestartRequest(roomId, playerId) {
        const room = this.rooms.get(roomId);
        if (!room)
            return;
        room.state = 'playing';
    ...
    advanceTurnOrRound(room) {
        ...
        if (room.currentRound > maxRounds || allFinished) {
            ...
            GameStatsService.recordMatch(matchData).then((success) => {
                if (success)
                    console.log(`[PlumZeeManager] Historique sauvegardé avec succès pour le salon ${room.id}`);
            });
            // =========================================================
        }
    }
