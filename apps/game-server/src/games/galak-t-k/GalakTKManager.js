...
export class GalakTKManager {
    ...
    handleMakeMove(roomId, playerId, move) {
        ...
        if (move.action === 'MARK_CELL') {
            ...
            if (!position || !markStatus)
                throw new Error("Données de marquage invalides.");
            ...
            if (result.isStar) {
                ...
                if (player.starsFoundCount >= room.stars.length) {
                    ...
                    GameStatsService.recordMatch(matchData).then((success) => {
                        if (success)
                            console.log(`[GalakTKManager] Historique sauvegardé avec succès pour le secteur ${roomId}`);
                    });
                    // =========================================================
                }
                else {
    ...
    toClientRoom(room, requestingPlayerId) {
