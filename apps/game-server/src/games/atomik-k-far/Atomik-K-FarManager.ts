...
export class AtomikKFardEManager {
    private io: Server;
    private rooms: Map<string, AtomikKFardEGameRoom>; // <-- C'est cette map qui sera utilisée
    private gameLogics: Map<string, AtomikKFardELogic>; // Pour gérer la logique de chaque salle si besoin
    ...
    public createRoom(
        ...
        const ownerPlayer: AtomikKFardEPlayer = {
            id: ownerId,
            roomId: roomId,
     ...
     */
    private transformRoomToClientFormat(room: AtomikKFardEGameRoom, requestingPlayerId: string): AtomikKFardERoomToSend {
    ...
    public addPlayerToRoom(room: AtomikKFardEGameRoom, playerId: string, username: string): AtomikKFardEGameRoom {
        ...
        // CORRECTION: Utilisation de AtomikKFardEPlayer au lieu de Player générique
        const newPlayer: AtomikKFardEPlayer = {
            gameType: 'AtomikKFardE',
            roomId: room.id,
            status: 'connected',
            id: playerId,
    ...
    public handlePlayerJoin(roomId: string, username: string, socketId: string): AtomikKFardEGameRoom {
        ...
        if (!room) {
                ...
                nbPlayer: 'duo' as AtomikKFardENbPlayer, // Cast pour s'assurer du type littéral
                ...
                timePerRound: 60 as AtomikTimePerRound, // 60 secondes
                maxRounds: 10,
                scoreToWin: 100,
            };
    ...
    public resolveCurrentRound(room: AtomikKFardEGameRoom): AtomikKFardEGameRoom {
        ...
        for (const card of roundResult.cardsToStealFromPlayer1 || []) { // Ajout || [] pour sécurité
            ...
            if (stealingPlayer && stealingPlayer.hand) stealingPlayer.hand.push(card);
        }
    ...
    public handleRestartRequest(roomId: string, requestingPlayerId: string): AtomikKFardEGameRoom | undefined {
        ...
        // CORRECTION: Utilisation de AtomikKFardEPlayer au lieu de Player générique
        const hostPlayer: AtomikKFardEPlayer = { // On recrée l'objet joueur pour le joueur qui a demandé le redémarrage
            gameType: 'AtomikKFardE',
            roomId: roomId,
            status: 'connected',
            socketId: '',
    ...
    private endRound(roomId: string): void {
        ...
        if (room.gameOptions.nbPlayer === 'duo' && room.gameOptions.teamMode !== 'Random') { // Gère le mode équipe
            ...
            const allSubmittedGrids: Record<string, AtomikGrid> = {};
        ...
        if (room.gameOptions.nbPlayer === 'duo' && room.gameOptions.teamMode !== 'Random') {
            ...
            const p1 = room.players[0];
            const p2 = room.players[1];
            if (p1) room.scores[p1.id] += roundResult.player1TotalScore;
            if (p2) room.scores[p2.id] += roundResult.player2TotalScore;
            // Mise à jour des scores individuels des joueurs pour chaque tour dans l'interface Player
            if (p1) p1.score = room.scores[p1.id];
            if (p2) p2.score = room.scores[p2.id];
    ...
    private determineGameWinner(room: AtomikKFardEGameRoom): string | null {
        if (room.gameOptions.nbPlayer === 'duo' && room.gameOptions.teamMode !== 'Random') {
            if (room.team1Score > room.team2Score) return 'Team1';
            if (room.team2Score > room.team1Score) return 'Team2';
            return null; // Égalité des équipes
        } else {
            // Pour le mode 1v1 ou sans équipe, compare les scores des joueurs
            const p1 = room.players[0];
            const p2 = room.players[1];
            if (!p1 || !p2) return null; // Pas assez de joueurs
    ...
    public toClientRoom(room: AtomikKFardEGameRoom): AtomikKFardERoomToSend {
        ...
        const clientRoom: AtomikKFardERoomToSend = {
            bombPropagationOrigin: room.bombPropagationOrigin,
            cafardBombPlayer1PropagationOrigin: room.cafardBombPlayer1PropagationOrigin,
            cafardBombPlayer2PropagationOrigin: room.cafardBombPlayer2PropagationOrigin,
            playerDisconnectTimers: room.playerDisconnectTimers,
