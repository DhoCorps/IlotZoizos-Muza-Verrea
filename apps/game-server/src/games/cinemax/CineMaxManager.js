...
export class CineMaxManager {
     ...
     */
    processDifficultySelection(room, player, difficulty) {
        if (!player.pendingDifficultyChoice)
            return;
     ...
     */
    processQuestionSolving(room, player, answer) {
     ...
     */
    processBuzzerHit(room, player, guessedTitle) {
    ...
    endRound(roomId, winnerId) {
        ...
        if (maxScoreReched || maxRoundsReached) {
            ...
            GameStatsService.recordMatch(matchData).then((success) => {
                if (success)
                    console.log(`[CineMaxManager] Historique sauvegardé avec succès pour le salon ${roomId}`);
            });
     ...
     */
    generateQuestionFromPool(movieData, difficulty) {
        ...
        return {
            ...
            options: difficulty === 'TEXT' ? [] : ["Acteur A", "Acteur B", "Brad Pitt", "Acteur C"].slice(0, difficulty),
            correctAnswer: "Brad Pitt"
        };
    }
    getMaxPlayers(nbPlayerSetting) {
