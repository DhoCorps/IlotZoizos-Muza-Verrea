...
export class WikiOracleManager {
    ...
    handleSubmitAnswer(roomId, playerId, answer) {
        const room = wikiOracleRooms.get(roomId);
        if (!room || room.state !== 'playing' || !room.currentQuestion)
            return;
    ...
    getRoom(roomId) { return wikiOracleRooms.get(roomId); }
    deleteRoom(roomId) { wikiOracleRooms.delete(roomId); }
